...
async function computeMacroSignals(): Promise<GetMacroSignalsResponse> {
  ...
  // All 6 calls in a single Promise.allSettled — Yahoo calls still serialize
  // via the internal yahooGate() in fetchJSON, but the non-Yahoo calls
  // (fearGreed, mempoolHash) overlap with Yahoo gate waits.
