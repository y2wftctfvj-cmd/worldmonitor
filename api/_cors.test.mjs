...
function makeRequest(origin) {
  ...
  return new Request('https://worldmonitor-two-kappa.vercel.app/api/test', { headers });
}
...
test('rejects unrelated external origins', () => {
  ...
  const cors = getCorsHeaders(req);
  assert.equal(cors['Access-Control-Allow-Origin'], 'https://worldmonitor-two-kappa.vercel.app');
});
